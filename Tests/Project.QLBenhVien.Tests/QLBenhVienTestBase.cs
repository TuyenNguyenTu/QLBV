...
namespace Project.QLBenhVien.Tests
{
    public abstract class QLBenhVienTestBase : AbpIntegratedTestBase<QLBenhVienTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected QLBenhVienTestBase()
        {
        ...
        protected void UsingDbContext(Action<QLBenhVienDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<QLBenhVienDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<QLBenhVienDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<QLBenhVienDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<QLBenhVienDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<QLBenhVienDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<QLBenhVienDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<QLBenhVienDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<QLBenhVienDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<QLBenhVienDbContext, Task<T>> func)
        {
            T result;
